                                    ...
                                    <div class="row">
                                        <div class="col-6 col-lg-3">
                                            <figure class="product-media">
                                                ...
                                                <a href="{{route('get_activity',$activite->slug)}}">
                                                    <img src="{{config('fashion.file_path').$activite->featured_image}}" alt="{{$activite->name}}" class="product-image">
                                                </a>
                                        ...
                                        <div class="col-6 col-lg-2 order-lg-last">
                                            <div class="product-list-action">
                                                <div class="product-price">
                                                     <span id="amount">{{number_format($activite->price)}}.00</span> 
                                                     <span id="amount_code"> AED</span>
                                                </div><!-- End .product-price -->

                                                <a href="{{route('get_activity',$activite->slug)}}" class="btn-product btn-cart" style="border-color: #001433;"><span>add to cart</span></a>
                                            </div><!-- End .product-list-action -->
                                        </div><!-- End .col-sm-6 col-lg-3 -->

                                        <div class="col-lg-7">
                                            <div class="product-body product-action-inner">
                                                <add-to-wishlist-button product-id="{{ $activite->id }}" :user-id="{{ Auth::user()->id ?? 0 }}"></add-to-wishlist-button>
                                                <div class="product-cat">
                                                    <a href="javascript::void()">{{$activite->activity_type}}</a>
                                                </div><!-- End .product-cat -->
                                                <h3 class="product-title"><a href="{{route('get_activity',$activite->slug)}}">{{$activite->name}}</a></h3><!-- End .product-title -->

                                                <div class="product-content">
                                                    <p>{!! $activite->about_experience !!}</p>
